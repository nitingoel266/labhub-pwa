...
const HeaterInitialPage = () => {
    ...
    return <div role="alert" aria-labelledby="dialog_label" aria-describedby="screen_desc" className={styles.TopWrapper} style={{height:window.innerHeight-150}}>
            <div aria-label={status?.heaterConnected ? 'Heater is Connected' : 'You must plug in both the heater and power connected before you can proceed'} className={styles.PimaryText}>{status?.heaterConnected ? 'Heater is Connected' : 'You must plug in both the heater and power cord before you can proceed'}</div>
